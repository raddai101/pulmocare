    ...
    if (!security_verify_csrf($_POST['_token'] ?? '')) {
        ...
        if ($tab === 'profile') {
            if (!empty($_FILES['avatar']['tmp_name'])) {
                ...
                else { html_set_flash('success', 'Avatar mis à jour.'); $user = auth_current_user(); }
            }
...
.profile-hero__avatar{width:88px;height:88px;border-radius:50%;object-fit:cover;border:3px solid rgba(59,130,246,.3);display:block}
  ...
  <main class="content">
    ...
    <div class="profile-hero">
      <form method="POST" enctype="multipart/form-data" id="avatarForm" style="display:contents">
        ...
        <label class="profile-hero__avatar-wrap" title="Changer la photo">
